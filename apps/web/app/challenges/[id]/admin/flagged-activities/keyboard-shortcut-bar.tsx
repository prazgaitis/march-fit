...
export function KeyboardShortcutBar() {
  ...
  const hasSelection = !!params.activityId;
  ...
  return (
    <div className="flex flex-shrink-0 items-center gap-4 border-b border-zinc-800 bg-zinc-900/50 px-4 py-1.5 text-[10px]">
      <Shortcut
        ...
        label="navigate"
      />
      <Shortcut keys={<Kbd>R</Kbd>} label="resolve / reopen" />
      <Shortcut keys={<Kbd>C</Kbd>} label="comment" />
      <Shortcut keys={<Kbd>E</Kbd>} label="edit" />
      <Shortcut
        keys={<Kbd>{isMac ? "⌘" : "Ctrl"}+↵</Kbd>}
        label="submit"
      />
    </div>
  );
