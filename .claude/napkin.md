...
| 2026-03-02 | self | Bulk import rewrite accidentally changed the new wrapper file to self-import/export (`@/lib/convex-auth-react` -> itself), creating recursive module resolution | Exclude newly-created wrapper files from global import-rewrite loops, then run a direct sanity grep for self-references immediately after codemods |
| 2026-03-02 | self | Ran combined startup command (`pwd && ls -la ...`) before reading napkin in isolation at session start | Make the first command exactly `cat .claude/napkin.md`; run all other exploration commands only after that completes |
...
| 2026-03-02 | self | During `git rebase` with stashed dirty state, `git stash pop` failed on `packages/backend/_generated/api.d.ts` because rebased commits changed the same generated file (`would be overwritten by merge`) | After rebase, inspect stashed-vs-current generated file diffs before forcing pop; keep rebased generated output if it supersedes older local generator state |
| 2026-03-03 | self | Ran shell commands against `apps/web/app/challenges/[id]/...` and `(...)/(dashboard)` paths without quotes, causing zsh glob errors (`no matches found` / `number expected`) | Always single-quote paths containing `[]` or `()` in shell commands, including `rg`, `find`, and `nl` |
...
| 2026-03-01 | self | Ran `sed` on `apps/web/app/challenges/[id]/(dashboard)/dashboard/page.tsx` without quoting and zsh expanded glob chars | Always single-quote route paths containing `[]` or `()` before shell commands |
| 2026-03-01 | self | Ran Vitest with two `-t` flags and command failed (`Expected a single value for option -t`) | Use a single `-t` pattern regex (or separate invocations) for targeted test runs |
