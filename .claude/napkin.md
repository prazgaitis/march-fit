...
| 2026-02-21 | self | Assumed existing scripted test credentials would still authenticate in production Playwright flow; session redirected back to sign-in | Treat embedded creds as potentially stale and verify auth first; request fresh test auth/session when E2E reproduction is blocked |
| 2026-02-21 | self | Playwright sign-up appeared flaky when only checking static page state; I missed that successful sign-up is best confirmed by waiting for URL transition | For auth E2E, assert success via `waitForURL('**/challenges**')` and capture non-GET auth requests rather than relying on immediate page text |
| 2026-02-21 | self | Production sign-up endpoint intermittently returns 500 in automation runs, causing brittle e2e failures | Add retry logic with fresh emails and treat sign-up response status + authenticated `/challenges` access as the success criteria |
